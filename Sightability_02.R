...
# 2.6 - 2021 ####
# All data
est.2021 <- Sight.Est(observed ~ 1, odat = subset(obs,year == 2021), sdat = exp,
...
colnames(tau.hats) <- c(names(temp$est), "LCL", "UCL")
tau.hats<-round(tau.hats, 0)
mHT.strat.2021 <- print(format(tau.hats, big.mark = ","), justify = "left", quote = FALSE)
...
obs.voc <- obs %>%
  filter(!is.na(voc))
# All data
est.2021.voc <- Sight.Est(observed ~ voc, odat = subset(obs.voc,year == 2021), sdat = exp,
                          sampinfo = subset(sampinfo, year == 2021))  
...
for(i in 1:length(eff.2021$ID)){
  ...
  temp <- Sight.Est(observed ~ voc, odat = tempobs, sdat = exp, tempsamp)
  temp.summary <- summary(temp)
  tau.hats[i,1:5] <- temp$est
  tau.hats[i,6] <- as.numeric(gsub(",","",temp.summary$lcl))
  tau.hats[i,7] <- as.numeric(gsub(",","",temp.summary$ucl))
...
colnames(tau.hats) <- c(names(temp$est), "LCL", "UCL")
tau.hats<-round(tau.hats, 0)
mHT.strat.2021.voc <- print(format(tau.hats, big.mark = ","), justify = "left", quote = FALSE)
mHT.strat.2021.voc <- as.data.frame(tau.hats) %>%
  select(tau.hat, LCL:UCL, everything())

setwd("C:/Users/TBRUSH/R/Elk_sightability/out")

save(list = c("mHT.all.2021", "mHT.strat.2021", "mHT.all.2021.voc", "mHT.strat.2021.voc"), file="mHT_output.RData")
