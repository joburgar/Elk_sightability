...
eff %>% select(EPU, ID, surveyed_2021, surveyed_2022) %>% print(eff)
```
...
sight.dat %>% group_by(z.tilde) %>% summarize(mean_x = mean(x.tilde))
```
...
scalar.dat <- scalar.dat %>%
  mutate(R = as.double(nrow(sight.dat)),
         ...
         ny1 = as.double(nrow(oper.dat[oper.dat$yr==1,]))
  )
```
